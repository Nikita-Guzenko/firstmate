...
A `done:` line in either mode that carries no PR URL is not a completion and never closes the task; `bin/fm-classify-lib.sh`'s "done: is not a completion without the PR" section owns that rule, and the drain's `REJECTED DONE` section names the task and the steer to send.
