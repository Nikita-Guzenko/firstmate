...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|antigravity) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
render_snippet() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    line=${line//__FM_PI_LIVE_STATUS_EXT__/$pi_live_status_ext}
...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    antigravity)
      printf '%s%s\n' "$prefix" 'resume supervision with bin/fm-watch-arm.sh as its own Antigravity background task, never shell &.'
      ;;
    codex)
      printf '%s%s%s%s\n' "$prefix" 'resume supervision with a foreground checkpoint: bin/fm-watch-checkpoint.sh --seconds ' "$checkpoint_seconds" '.'
      ;;
    pi)
      printf '%s%s%s%s%s%s%s%s\n' "$prefix" 'resume supervision through the Pi extension command /fm-watch-arm-pi or restart Pi with -e ' "$pi_live_status_ext" ' -e ' "$pi_turnend_ext" ' -e ' "$pi_ext" ' if the extension is not loaded.'
      ;;
    opencode)
