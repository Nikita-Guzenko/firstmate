...
2. Confirm the Pi primary was launched with `-e __FM_PI_LIVE_STATUS_EXT__ -e __FM_PI_TURNEND_EXT__ -e __FM_PI_EXT__`.
...
6. If the extension reports a watcher failure, drain queued wakes, inspect the failure text, and restart Pi with the required extensions loaded if needed.
7. Never use shell `&` for watcher supervision.

The turn-end guard extension lives at `__FM_PI_TURNEND_EXT__`.
The live status extension lives at `__FM_PI_LIVE_STATUS_EXT__`.
The generated watcher extension lives at `__FM_PI_EXT__`.
`bin/fm-session-start.sh` creates or refreshes the generated watcher extension for Pi primaries and reports when the running Pi session has not loaded all required extensions.
