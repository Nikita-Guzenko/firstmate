...
3. Run `bin/fm-watch-arm.sh` as its own Antigravity background task.
...
9. If a forced restart is genuinely needed, run `bin/fm-watch-arm.sh --restart` through the same Antigravity background task mechanism.
10. Do not send idle progress while the watcher is parked.

Antigravity's background task completion is the wake mechanism.
