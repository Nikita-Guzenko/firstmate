...
firstmate's always-loaded operating core for the orchestrator agent itself is [`AGENTS.md`](../AGENTS.md); this is the human-facing companion.
...
For ship and scout work, `fm-spawn.sh` refuses to launch unless the resolved task path is a real git worktree root that is distinct from the project primary checkout AND shares the target project's git common dir, so a foreign repository (another firstmate home's own checkout, reachable because every home shares one tmux session) cannot be accepted as the task's worktree.
