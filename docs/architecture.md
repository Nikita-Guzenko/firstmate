...
For ship and scout work, `fm-spawn.sh` refuses to launch unless the resolved task path is a real git worktree root that is distinct from the project primary checkout AND positively belongs to the target project - it shares the project's git common dir, or (for backends whose worktrees are independent clones) has an identical `origin` URL - so a foreign repository (another firstmate home's own checkout, reachable because every home shares one tmux session) cannot be accepted as the task's worktree.
