...
A crew's own `done:` line is prose about its own commit, and seven sessions closed a task on one that had no PR behind it - including the session that wrote this rule, which reported `done:` on its own unpushed commit hours after implementing the refusal - so `fm-classify-lib.sh` also owns a single condition that separates the two: in a PR-delivering mode a `done:` carrying no PR URL is not a completion, `fm-wake-drain.sh` re-surfaces it in a REJECTED DONE section with the steer to send, `fm-crew-state.sh` reports it as `blocked`, and the captain-relevant scan renders it with its refusal instead of as a completion; `local-only`, scout, and charter work have no PR by design and are untouched.
