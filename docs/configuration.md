...
FM_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=2 # seconds fm-teardown.sh waits before retrying a worktree return that failed - whether on a git lock or with no lock present (a transient pool flake)
