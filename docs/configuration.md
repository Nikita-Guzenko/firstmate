...
For Pi secondmate launches, `fm-spawn.sh` generates the secondmate home's `state/fm-primary-pi-watch.ts` bridge and starts Pi with that bridge, `.pi/extensions/fm-primary-turnend-guard.ts`, and `.pi/extensions/fm-live-status.ts`.
...
The canonical schema, array `use` form, `select` strategies, and quota-balanced fallback contract live in the agent-only `crew-dispatch` skill (`.agents/skills/crew-dispatch/SKILL.md`); `AGENTS.md` section 4 keeps only the intake precedence and the "consult rules, pass explicit flags" contract.
...
FM_SEND_MAX_INLINE_CHARS=600   # fm-send refuses an inline message longer than this (collapses into an unsubmittable paste); route it through a file plus a short pointer
FM_SEND_MAX_INLINE_LINES=8     # fm-send refuses an inline message with more lines than this, same reason
