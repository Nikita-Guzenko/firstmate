...
- Codex registers a `Stop` hook in `.codex/hooks.json`, anchors the executable to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, clears the verified identity markers of other harnesses at that Codex-owned boundary, and passes the original payload to the shared guard.
  The boundary cleanup is required when a native Codex process is started below a Pi, Claude, Grok, or Cursor process and inherits its exported marker; the hook then lets process ancestry identify the nearer Codex process instead of selecting the outer harness's repair protocol.
